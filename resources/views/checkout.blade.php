...
<script>
    document.getElementById('place-order-btn').addEventListener('click', function (e) {
        ...
        .then(data => {
            console.log('Midtrans Payload:', data.payload);  // debug payload
            if (data.snap_token && data.snap_token.original.token) {
                window.snap.pay(data.snap_token.original.token, {
                    ...
                    onPending: function(result){
                        ...
                        window.location.href = `/order-confirmation?status=pending&order_id=${result.order_id}`;
                    },
                    onError: function(result){
                        console.error('Payment error:', result);
                        window.location.href = `/order-confirmation?status=error&order_id=${result.order_id}`;
                    },
                    onClose: function(){
